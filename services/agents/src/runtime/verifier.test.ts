/**
 * How an agent runs the repository's suite.
 *
 * These run real commands in a real temp directory rather than stubbing
 * runCommand. That is the point of them — what is being tested is that setup
 * runs once and not once per call, that a failed setup is retried, and that a
 * non-zero exit becomes a `passed: false` verification rather than an exception.
 * All three are properties of process handling, and stubbing the process
 * handling away would leave the tests asserting their own stub.
 *
 * Moved here with createVerifier itself, which the reviewer shares.
 */
...
import type { RepoManifest } from '@cloud-harness/shared';

import { createVerifier, type Verifier, type VerifierOptions } from './verifier.js';
...
describe('createVerifier', () => {
   ...
   * value the caller can act on, with the output that says why. The reviewer
   * needs this even more than the implementer did: a failing suite is the one
   * finding it does not have to persuade anyone of.
   */
   ...
   * and loses the whole run, where a sentence leaves it able to finish and say
  ...
  it('caps the model at six test runs and tells it what to do instead', async () => {
    ...
    assert.match(response, /unable to verify/);
  });

  /**
   * The reviewer takes a smaller budget than the implementer: its loop reads and
   * runs, it does not write and re-run, and a reviewer asking for a sixth run is
   * not reviewing.
   */
  it('takes the budget from the caller', async () => {
    const verifier = build({ testCommand: 'true' }, { maxRuns: 2 });

    await runTests(verifier);
    await runTests(verifier);

    assert.match(await runTests(verifier), /budget of 2 is spent/);
  });
